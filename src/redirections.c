...
int	call_redirections(t_cmd *cmd)
{
    ...
    handle_output_redirection(cmd->redirs, &cmd->redirc, &redirs.output_file, &redirs.append_file);
    handle_heredoc_redirection(cmd->redirs, &cmd->redirc, &redirs.delimiter, &redirs.input_file);
